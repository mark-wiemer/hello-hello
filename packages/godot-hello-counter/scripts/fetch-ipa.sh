#!/usr/bin/env bash
# Trigger/await the Godot Hello Counter iOS CI build and download the unsigned .ipa locally.
#
...
# HelloCounter.ipa into <package>/build/ipa/.
#
...
workflow="godot-hello-counter-ios.yml"
artifact="HelloCounter-ios-ipa"
ipa_name="HelloCounter.ipa"
dispatch=0
out_dir=""
...
ipa="$(find "$out_dir" -name "$ipa_name" -type f | head -n 1)"
if [ -z "$ipa" ]; then
  echo "Artifact downloaded but no $ipa_name found under $out_dir." >&2
  exit 1
fi
