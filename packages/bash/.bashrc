...
# CoPy Recursive Dist to Void
alias cprdv='cp -r dist ~/.var/app/org.luanti.luanti/.minetest/games/void'
...
luanti_dev() {
    if [ $# -eq 0 ]; then
        ...
        echo "then launch Luanti from Flatpak"
        echo "Usage: luanti_dev dest [source]"
        echo "Examples:"
        echo "  luanti_dev mwvl # copy Luanti files in cwd to mwvl game"
        echo "  luanti_dev dist void # copy Luanti files in ./dist to void game"
        return 1
    fi
    
    local targets=("menu" "mods" "textures" "game.conf" "minetest.conf")
    
    local dest="$1"
    local source="${2:-.}"
    for target in "${targets[@]}"; do
        eval mkdir -p $luanti_games_relative_path/$dest
        eval cp -r $source/$target ~/.var/app/org.luanti.luanti/.minetest/games/$dest/$target
        done
...
#region pnpm
export PNPM_HOME="/home/markw/.local/share/pnpm"
case ":$PATH:" in
  *":$PNPM_HOME:"*) ;;
