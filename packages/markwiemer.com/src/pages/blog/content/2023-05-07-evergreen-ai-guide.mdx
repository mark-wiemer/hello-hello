---
postTitle: "ChatGPT and generative AI: An evergreen guide"
postDate: "2023-05-07"
original: "https://markwiemer.medium.com/chatgpt-and-generative-ai-an-evergreen-guide-89c67ba0fba4"
postType: "notes"
lastUpdated: "2026-06-05"
---

import CaptionedImage from "@/components/CaptionedImage.astro";
import AiGlowingMap from "@/assets/2023-05-07-ai-glowing-map.webp";

## A friendly, frequently-updated index of all my AI articles

<CaptionedImage
  src={AiGlowingMap}
  alt="A glowing map held outdoors in a forest at night, first-person view."
  caption={`Like AI art, this guide will not be perfect, but hopefully it will be illuminating. Made with Midjourney.`}
  eager
/>

Wow, there sure is a lot of AI news nowadays. If you've been drinking from the firehose like me, you may be frustrated by the lack of definitions, clarity, and simple explanations of all of this new technology. I sure am. It took me months to figure out what a parameter was, and I'm a software engineer! (A parameter, or weight, is just a number that is tweaked by a machine learning algorithm to affect its model.)

Since March, I've been publishing articles about AI, but each post builds on the last, and I don't want to contribute to the confusion. This post will serve as a friendly digital index. It'll have keywords for easy [searching on a single page](https://www.howtogeek.com/671304/how-to-quickly-search-for-text-on-the-current-web-page/), and I'll keep it updated as I learn and write more.

If you're ever confused, I'm here to help! Reach out in the comments, on [LinkedIn](https://www.linkedin.com/in/markwiemer/), or however you feel comfortable. AI scared me a ton for months, and I thought I missed the boat when ChatGPT was 2 months old and I had never used it. But it wasn't too late — and it still isn't! Whenever you're reading this, whatever you know, know this: Learning is forever, and the best time to start is today. Let's dive in.

---

This article mentions Microsoft, my employer. I wrote this article in my free time and all opinions are my own.

---

## About me

A personal autobiography that provides great context about my childhood experience with technology.

Keywords: profile, background.

- [Who am I anyway? A 5-minute autobiography](/blog/who-am-i)

---

## Let's learn about artificial intelligence

My main article series, written for anyone regardless of their tech experience. If you can open your email on a phone, you can understand these articles!

**Here's the intro**, in which I define AI, explain machine learning, write about OpenAI and their partnership with Microsoft, chatbot tools like Microsoft Bing and Google Bard, and announcements for Microsoft 365 Copilot, Khanmigo by Khan Academy, and more.

Keywords: GPT-3.5, GPT-4, GPT-n, model, algorithm, AI effect, chatbot tools, training, natural language.

- [Let's learn about artificial intelligence](/blog/lets-learn-ai)

**In the second entry**, I break down the technical terms behind the buzzwords, show how products like Bing and Bard are made, and explain how they're different than ChatGPT.

Keywords: Intelligence, smart, model, algorithm, kitchen analogy, data, predictions, reinforcement learning from human feedback (RLHF), large language model (LLM), product, grounding, hallucination, neural network, transformer.

- [AI is a buzzword. Here are the real words to know](/blog/ai-buzzword-replacement)

**The third entry** introduces prompt engineering, which is the foundation of grounding and the simplest way to make these products more useful.

- [Unlock the power of ChatGPT with prompt engineering](/blog/ai-prompt-engineering)

**The fourth entry** clarifies the difference between the large language model and the rest of ChatGPT, an important distinction as other companies build related technology.

- [ChatGPT was never just a language model, despite OpenAI's claims](/blog/chatgpt-not-model)

**The fifth entry** covers how AI interacts with the rest of the digital world. It covers plugins, structured data, manifests, and natural language with a weather plugin example. This is the core of the plugin loop that is still used in 2026.

- [ChatGPT, beyond the model: Plugins and intent recognition](/blog/chatgpt-plugins-intent)

---

## AI Updates

Briefs about the most relevant news for AI users like you and me.

**The first AI Update** covers product announcements from Big Tech.

Keywords: Bard, Bing, LLaMA, Dynamics 365 Copilot, GPT-4, PaLM API & MakerSuite, Google Workspace, Microsoft 365 Copilot, Semantic Kernel, GitHub Copilot X, AWS Generative AI Accelerator.

- [AI Updates: The 10 biggest announcements since ChatGPT](/blog/ai-updates-chatgpt)

**In the second installment**, I write about more open-source advancements and different ways new AI is being used.

Keywords: AI agents, WolframAlpha, BabyAGI, AgentGPT, Khanmigo, HuggingGPT, Microsoft JARVIS, Hugging Face, HustleGPT, GPT4All, Stanford Alpaca, ChatLLaMA, Vicuna, Koala, anthropomorphism.

- [AI Updates: The Auto-GPT rocket, LLaMA's cheap children, and more](/blog/ai-updates-autogpt)

---

## Tangents on emotion, anxiety, and real life

These are more dramatic, non-informative posts where I share my feelings as I learn about tech. I've felt lost, frustrated, confused, triumphant, scared, excited, nervous, and more. I hope we all remember that we're all human, and sharing these articles is my personal reminder.

- [Yesterday I learned the secrets of the universe. Today I struggled to highlight a PDF. How did we get here?](/blog/yesterday-learned-secrets)
- [AI is cool, but why do I feel like a failure?](/blog/ai-imposter-syndrome)
- [There are so many worlds to explore](/blog/worlds-to-explore)
- [I don't believe everything I write](/blog/self-doubt-writings)
- [What it means to be a person](/blog/being-a-person)
- [Despite everything, depression lingers](/blog/spiteful-depression-lingers)

---

## Less-organized notes

In 2026, I started [GenAI notes](/blog/genai) to publish what I'd learned from years working on closed-source AI stuff. It's a living document like this one that focuses less on structure and more on information. Notes will eventually turn into articles, which will be indexed here for later reference.

---

Check back often as I continue to update this index with more articles!

And, as always, thank you for reading. What would you like to learn next? How can I help? Let me know in the comments! 🤓
